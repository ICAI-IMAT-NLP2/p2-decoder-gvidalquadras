...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        scores = scores / torch.sqrt(torch.tensor(dim_k, dtype=torch.float32, device=scores.device))

        # Apply mask
        if mask is not None:
            # masked_fill -> copies values of tensor 2 into tensor 1 in positions where the mask is true
            # In this case, set scores for future tokens to -inf (we only want to pay attention to past tokens)

            # turn mask to boolean
            causal_mask = (mask == 0.0) # true if element is 0.0 (is a future token to which we dont want to pay attention)
            
            scores = scores.masked_fill_(causal_mask, float('-inf')) 
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.head_dim = d_model // num_attention_heads #d_k = d_q = d_v = head_dim

        self.heads = nn.ModuleList([
            AttentionHead(d_model, self.head_dim, self.head_dim, self.head_dim)
    ...
    def forward(self, hidden_state, mask=None):
        ...
        # Same as encoder, now with mask in head
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int, intermediate_size: int):
        ...
        # Normalization over last dimension -> d_model (batch_size, seq_len, d_model)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        # Apply layer normalization and then apply masked multi-head attention
        normalized_input = self.layer_norm_1(x)
        hidden_state = self.self_attention(normalized_input, mask)

        # Add a residual connection from the input to the output
        x = x + hidden_state

        # Apply layer normalization and then apply feed-forward network
        normalized_attention_output = self.layer_norm_2(x)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        # Stack of TransformerEncoderLayer instances
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        float_mask = torch.tril(torch.ones(seq_len, seq_len, device=device)) # turn into boolean in attentionhead (becasue of how tests work)

        float_mask = float_mask.unsqueeze(0) # -> (1, seq_len, seq_len)
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        decoder_output = self.transformer_decoder(input_ids) # -> (batch_size, seq_len, d_model)
